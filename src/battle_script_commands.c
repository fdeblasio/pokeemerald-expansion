...
const ALIGNED(4) u8 gBattlePalaceNatureToMoveGroupLikelihood[NUM_NATURES][4] =
{
    ...
    [NATURE_BRAVE]   = PALACE_STYLE(70, 15, 80, 15), // 15%,  5%
    [NATURE_ADAMANT] = PALACE_STYLE(70, 15, 70, 15), // 15%, 15%
    [NATURE_NAUGHTY] = PALACE_STYLE(20, 10, 40, 10), // 70%, 50%
    [NATURE_BOLD]    = PALACE_STYLE(30, 20, 32, 58), // 50%, 10%
    [NATURE_DOCILE]  = PALACE_STYLE(22, 56, 22, 56), // 22%, 22%
    [NATURE_RELAXED] = PALACE_STYLE(25, 15, 75, 15), // 60%, 10%
    [NATURE_IMPISH]  = PALACE_STYLE( 6, 25, 28, 55), // 69%, 17%
    [NATURE_LAX]     = PALACE_STYLE(35, 10, 29,  6), // 55%, 65%
    [NATURE_TIMID]   = PALACE_STYLE(42, 28, 20, 50), // 30%, 30%
    [NATURE_HASTY]   = PALACE_STYLE(58, 37, 12,  6), //  5%, 82%
    ...
    [NATURE_MODEST]  = PALACE_STYLE(55, 25, 60, 34), // 20%,  6%
    [NATURE_MILD]    = PALACE_STYLE(44, 50, 34,  6), //  6%, 60%
    [NATURE_QUIET]   = PALACE_STYLE(22, 56, 56, 22), // 22%, 22%
    ...
    [NATURE_SASSY]   = PALACE_STYLE(54,  6, 72,  6), // 40%, 22%
...
static const u8 sBattlePalaceNatureToFlavorTextId[NUM_NATURES] =
{
    ...
    [NATURE_BRAVE]   = B_MSG_GLINT_IN_EYE,
    [NATURE_ADAMANT] = B_MSG_EAGER_FOR_MORE,
    ...
    [NATURE_SASSY]   = B_MSG_GLINT_IN_EYE,
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                #else
                    if (gSaveBlock2Ptr->expShare && gBattleMoveDamage == 0)
                        gBattleMoveDamage += gExpShareExp;
                #endif
