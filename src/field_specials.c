...
bool32 ShouldDoDuncanCall(void)
{
    if (FlagGet(FLAG_ENABLE_FIRST_DUNCAN_POKENAV_CALL))
    {
        ...
        case MAP_TYPE_OCEAN_ROUTE:
            if (++(*GetVarPointer(VAR_DUNCAN_CALL_STEP_COUNTER)) < 250)
                return FALSE;
            break;
        default:
            return FALSE;
        ...
        if (myLinkPlayerNumber != i)
        {
            ...
            case VERSION_SAPPHIRE:
                if (gLinkPlayers[i].gender == 0)
                    linkSpriteId = OBJ_EVENT_GFX_LINK_RS_KYLE;
                else
                    linkSpriteId = OBJ_EVENT_GFX_LINK_RS_KELLI;
                break;
            case VERSION_EMERALD:
                if (gLinkPlayers[i].gender == 0)
                    linkSpriteId = OBJ_EVENT_GFX_RIVAL_KYLE_NORMAL;
                else
                    linkSpriteId = OBJ_EVENT_GFX_RIVAL_KELLI_NORMAL;
                break;
            default:
                if (gLinkPlayers[i].gender == 0)
                    linkSpriteId = OBJ_EVENT_GFX_RIVAL_KYLE_NORMAL;
...
static void LoadLinkPartnerObjectEventSpritePalette(u16 graphicsId, u8 localEventId, u8 paletteNum)
{
    ...
    if (graphicsId == OBJ_EVENT_GFX_LINK_RS_KYLE ||
        graphicsId == OBJ_EVENT_GFX_LINK_RS_KELLI ||
        graphicsId == OBJ_EVENT_GFX_RIVAL_KYLE_NORMAL ||
        graphicsId == OBJ_EVENT_GFX_RIVAL_KELLI_NORMAL)
    {
        ...
        if (obj != OBJECT_EVENTS_COUNT)
        {
            ...
            switch (graphicsId)
            {
            case OBJ_EVENT_GFX_LINK_RS_KYLE:
                LoadPalette(gObjectEventPal_RubySapphireKyle, OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
                break;
            case OBJ_EVENT_GFX_LINK_RS_KELLI:
                LoadPalette(gObjectEventPal_RubySapphireMay, OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
                break;
            case OBJ_EVENT_GFX_RIVAL_KYLE_NORMAL:
                LoadPalette(gObjectEventPal_Kyle, OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
                break;
            case OBJ_EVENT_GFX_RIVAL_KELLI_NORMAL:
...
void CableCarWarp(void)
{
    if (gSpecialVar_0x8004 != 0)
        SetWarpDestination(MAP_GROUP_AND_NUM(ROUTE112_CABLE_CAR_STATION), WARP_ID_NONE, 6, 4);
    else
        SetWarpDestination(MAP_GROUP_AND_NUM(MT_CHIMNEY_CABLE_CAR_STATION), WARP_ID_NONE, 6, 4);
}
...
{
    return gMapHeader.mapLayout->secondaryTileset == &gTileset_KylesMaysHouse
        && (tileId == METATILE_KylesMaysHouse_KylePC_On
            || tileId == METATILE_KylesMaysHouse_KylePC_Off
            || tileId == METATILE_KylesMaysHouse_MayPC_On
            || tileId == METATILE_KylesMaysHouse_MayPC_Off);
}
...
static void PCTurnOnEffect_SetMetatile(s16 isScreenOn, s8 dx, s8 dy)
{
    ...
    if (isScreenOn)
    {
        ...
        else if (gSpecialVar_0x8004 == PC_LOCATION_KYLES_HOUSE)
            metatileId = METATILE_KylesMaysHouse_KylePC_Off;
        else if (gSpecialVar_0x8004 == PC_LOCATION_KELLIS_HOUSE)
            metatileId = METATILE_KylesMaysHouse_MayPC_Off;
    }
    else
    {
        ...
        if (gSpecialVar_0x8004 == PC_LOCATION_OTHER)
            ...
            metatileId = METATILE_KylesMaysHouse_KylePC_On;
        else if (gSpecialVar_0x8004 == PC_LOCATION_KELLIS_HOUSE)
            metatileId = METATILE_KylesMaysHouse_MayPC_On;
    }
...
static const u8 *const sScrollableMultichoiceOptions[][MAX_SCROLL_MULTI_LENGTH] =
{
    ...
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2] =
    {
        ...
        gText_RegirockDoll4BP,
        gText_RegiceDoll4BP,
        gText_RegisteelDoll4BP,
        gText_Exit
    },
    ...
    {
        gText_RisingVoltage1BP,
        gText_GrassyGlide1BP,
        gText_ExpandingForce1BP,
        gText_ZenHeadbutt1BP,
        gText_Scald1BP,
        gText_PsychoCut1BP,
        gText_Lunge1BP,
        gText_ThroatChop1BP,
        gText_SolarBlade1BP,
        gText_DualWingbeat1BP,
        gText_ScorchingSands1BP,
        gText_KnockOff1BP,
        gText_MegaPunch1BP,
        gText_MegaKick1BP,
        gText_Exit
    ...
    {
        gText_ElectricTerrain1BP,
        gText_GrassyTerrain1BP,
...
void SetBattleTowerLinkPlayerGfx(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (gLinkPlayers[i].gender == MALE)
            VarSet(VAR_OBJ_GFX_ID_F - i, OBJ_EVENT_GFX_KYLE_NORMAL);
        else
            VarSet(VAR_OBJ_GFX_ID_F - i, OBJ_EVENT_GFX_RIVAL_KELLI_NORMAL);
    }
}
...
u32 GetMartEmployeeObjectEventId(void)
{
    ...
    {
        { MAP_GROUP_AND_NUM(OLDALE_TOWN_MART),     LOCALID_OLDALE_MART_CLERK },
        { MAP_GROUP_AND_NUM(LAVARIDGE_TOWN_MART),  LOCALID_LAVARIDGE_MART_CLERK },
        { MAP_GROUP_AND_NUM(FALLARBOR_TOWN_MART),  LOCALID_FALLARBOR_MART_CLERK },
        { MAP_GROUP_AND_NUM(VERDANTURF_TOWN_MART), LOCALID_VERDANTURF_MART_CLERK },
        { MAP_GROUP_AND_NUM(PETALBURG_CITY_MART),  LOCALID_PETALBURG_MART_CLERK },
        { MAP_GROUP_AND_NUM(SLATEPORT_CITY_MART),  LOCALID_SLATEPORT_MART_CLERK },
        { MAP_GROUP_AND_NUM(MAUVILLE_CITY_MART),   LOCALID_MAUVILLE_MART_CLERK },
        { MAP_GROUP_AND_NUM(RUSTBORO_CITY_MART),   LOCALID_RUSTBORO_MART_CLERK },
        { MAP_GROUP_AND_NUM(FORTREE_CITY_MART),    LOCALID_FORTREE_MART_CLERK },
        { MAP_GROUP_AND_NUM(MOSSDEEP_CITY_MART),   LOCALID_MOSSDEEP_MART_CLERK },
        { MAP_GROUP_AND_NUM(SOOTOPOLIS_CITY_MART), LOCALID_SOOTOPOLIS_MART_CLERK },
        { MAP_GROUP_AND_NUM(BATTLE_FRONTIER_MART), LOCALID_BATTLE_FRONTIER_MART_CLERK }
    };
