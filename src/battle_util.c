...
bool32 TryRunFromBattle(enum BattlerId battler)
{
    ...
    else if (IsSemiInvulnerable(GetOppositeBattler(battler), CHECK_ALL)) {
        effect++;
    }
    else if (gBattleMons[GetOppositeBattler(battler)].status1 & (STATUS1_SLEEP || STATUS1_FREEZE)) {
        effect++;
    }
    else
    {
...
const u8 *AbsorbedByCharging(enum BattlerId battlerDef)
{
    //From Electromorphosis if (!gBattleStruct->unableToUseMove && IsBattlerTurnDamaged(battlerDef) && IsBattlerAlive(battlerDef))
...
bool32 TryFieldEffects(enum FieldEffectCases caseId)
{
    ...
    case FIELD_EFFECT_OVERWORLD_TERRAIN:   // terrain starting from overworld weather
        #define SET_TERRAIN(Terrain)                                                 \
            gFieldTimers.terrain = B_TERRAIN_##Terrain;                              \
        ...
        else if ((gMapHeader.mapLayoutId == LAYOUT_VERDANTURF_TOWN || gBattleEnvironment == BATTLE_ENVIRONMENT_LONG_GRASS) && gFieldTimers.terrain != B_TERRAIN_GRASSY)
        {
            SET_TERRAIN(GRASSY)
        }
        break;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (IsFieldMudSportAffected(ctx->moveType))
        UQ4_12_MULTIPLY(modifier, GetConfig(B_SPORT_DMG_REDUCTION) >= GEN_5 ? 0.33 : 0.5);
    ...
    case ABILITY_RIVALRY:
        if (AreBattlersOfSameGender(battlerAtk, battlerDef))
            UQ4_12_MULTIPLY(modifier, 1.25);
        else if (AreBattlersOfOppositeGender(battlerAtk, battlerDef))
            UQ4_12_MULTIPLY(modifier, 0.75);
        break;
    ...
    case HOLD_EFFECT_SOUL_DEW:
        if ((gBattleMons[battlerAtk].species == SPECIES_LATIAS || gBattleMons[battlerAtk].species == SPECIES_LATIOS)
            && ((B_SOUL_DEW_BOOST >= GEN_7 && (moveType == TYPE_PSYCHIC || moveType == TYPE_DRAGON))
             || (B_SOUL_DEW_BOOST < GEN_7 && IsBattleMoveSpecial(move))))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    ...
    case ABILITY_ORICHALCUM_PULSE:
        if (ctx->weather & B_WEATHER_SUN && IsBattleMovePhysical(move)
         && ctx->holdEffects[ctx->battlerAtk] != HOLD_EFFECT_UTILITY_UMBRELLA)
           UQ4_12_MULTIPLY(modifier, 1.3333);
        break;
    case ABILITY_HADRON_ENGINE:
